...
Đồ thị tri thức được xây dựng từ metadata phim, hồ sơ người dùng và tương tác user-item trong tập huấn luyện. Cần phân biệt rõ dữ liệu dùng cho KGE với dữ liệu sau khi lọc threshold của mô hình gợi ý:
...
Vì vậy, KGE không đơn giản là dùng "dataset sau khi lọc threshold". KGE dùng **train split trước khi lọc threshold**, sau đó dùng threshold để phân loại cạnh user-item. TransE học embedding sao cho mỗi bộ ba $(h,r,t)$ thỏa gần đúng:
